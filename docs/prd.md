...
**Then** a VLM generates a structured caption (components, wiring, connector positions, visible text, displays, and any state change from the previous capture) with no action from the user beyond the original button press, and the row becomes searchable.
...
| F-11 | Caption image via VLM at ingest, no user trigger required. Prompt is tuned for hardware benches: components, wiring, connector positions, visible text, displays, and any state change from the previous capture.                                                     |
